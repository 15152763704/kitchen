			...
			<view class="right">
				<scroll-view scroll-y
					...
					<view class="right-container" v-for="(item,index) in list" :key="index">
						<view class="category-name" :id="`cid${ item.shopTypeId }`">
	...
	export default {
		...
		methods: {
			...
			categoryClick(category, index) {
				this.$nextTick(() => {
					this.cIndex = `cid${ category.shopTypeId }`
					this.flag = true
				})
				this.cIndex = ''
				this.active = index;
			...
			itemClick (category, item) {
				uni.navigateTo({
					url: '/pages/search/search?selectTypeName=' + item.shopTypeName + '&selectTypeId=' + item.shopTypeId
				});
			},
			getValues () {
				let ids = []
		...
		mounted() {
			uni.getSystemInfo({
				success: res => {
					this.height = res.screenHeight -  44;
				}
			})
			this.getNodesInfo()
			setTimeout(() => {
