<template>
	<view class="common-car">
		<view class="empty-shop-car" v-if="isEmpty">
			...
			<!-- <view class="empty-shop-car-btn" @click="toShopping">
				<text>去逛逛</text>
			</view> -->
		</view>
		<view class="shop-car" v-else>
			...
			<view class="store-box" v-for="(itemq,indexq) in datas" :key="indexq">
				<view class="store-header">
					<view class="" v-if="itemq.userCarsVo.temp_checked == 2">
						<image src="../../static/select.png" class="checked-image" mode=""
							@tap="storeCheck(indexq,itemq.userCarsVo.temp_checked)"></image>
					</view>
					<view class="" v-else>
					...
					<view class="">
						<text class="itemq-name">{{itemq.userCarsVo.shop}}</text>
				...
				<view class="goodsInfo" v-for="(itemw,indexw) in itemq.userCarsVo.data" :key="indexw">
					<view class="">
					...
					<view class="goodsInfo-right">
						...
						<view class="goodsInfo-box">
							...
							<view class="">
								<text class="goods-price"><text style="font-size: 18rpx;">¥</text>{{itemw.money}}</text>
							</view>
						</view>
					</view>
				</view>
				...
				<view class="cost-count">
					...
					<view class="cost">
						...
						<view v-if="isCut" class="settlement-box">
							...
							<text @click="account(itemq)" class="settlement-btn-disabled" :class="perTotalArr[indexq] >= 0.01? 'settlement-btn': ''">
								去结算
								<!-- {{ perTotalArr[indexq] < 1?'差￥'+ (1 - perTotalArr[indexq]) +'起送': '去结算' }} -->
							</text>
						</view>
					</view>
				</view>
	...
	export default {
		...
		watch:{
			list(val){
				...
				if (val.length == 0) {
					...
					let _obj = JSON.stringify(val)
					this.datas = JSON.parse(_obj)
		...
		methods: {
			...
			goodsCheck(storeIndex, goodsIndex, goodsChecked) {
				...
				if (goodsChecked == 1) {
					this.datas[storeIndex].userCarsVo.data[goodsIndex].temp_checked = 2
				} else {
					this.datas[storeIndex].userCarsVo.data[goodsIndex].temp_checked = 1
				}
				// 判断是否该店铺全选
				let storeChecked = true
				this.datas[storeIndex].userCarsVo.data.map((item, index) => {
					if (item.temp_checked == 1) {
						storeChecked = false
					}
				})
				if (storeChecked == false) {
					this.datas[storeIndex].userCarsVo.temp_checked = 1
				} else {
					this.datas[storeIndex].userCarsVo.temp_checked = 2
				}

				// 判断是否全选
				let statisticsIndex = true
				this.datas.find((item, index) => {
					if (item.userCarsVo.temp_checked == 1) {
			...
			storeCheck(storeIndex, storeCheck) {
				if (storeCheck == 1) {
					this.datas[storeIndex].userCarsVo.temp_checked = 2
					this.datas[storeIndex].userCarsVo.data.find((item, index) => {
						item.temp_checked = 2
					})
				} else {
					this.datas[storeIndex].userCarsVo.temp_checked = 1
			...
			allCheck() {
				if (this.statisticsIndex) {
					this.datas.find((item, index) => {
						item.userCarsVo.temp_checked = 1
						item.userCarsVo.data.find((itemq, indexq) => {
					...
					this.datas.find((item, index) => {
						item.userCarsVo.temp_checked = 2
						item.userCarsVo.data.find((itemq, indexq) => {
			...
			cut() {
				...
				if(!this.isCut) {
					...
					let _obj = JSON.stringify(this.list)
					this.datas = JSON.parse(_obj)
					this.statisticsIndex = true;
					this.statistics()
					// this.datas = this.list;
			...
			account(itemq) {
				// console.log(itemq)
				let judge = this.jdgSelect(itemq)
				if (judge) {
					// this.$emit('accounts', this.datas)
					let idArr = [];
					itemq.userCarsVo.data.find((item, indexq) => {
						if (item.temp_checked == 2) {
			...
			delect() {
				...
				if (judge) {
					...
					// console.log(goodIdArr.join(";"));
					this.$emit('delect', goodIdArr.join(";"))
				} else {
					uni.showToast({
