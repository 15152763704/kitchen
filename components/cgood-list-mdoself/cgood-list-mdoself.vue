		...
		<view class="goods-list" v-for="(item,index) in shopList" :key="index" @click="gotoStore(item)">
			...
			<view class="goods-info">
				...
				<view class="goods-info-4">
					<!-- <view class="goods-info-fullreduce" v-if="item.selfMadeGoodCouponCard">
						<template v-for="(value,iindex) in item.selfMadeGoodFullReduceArr.slice(0, 3)">
							<text class="fullreduce-text">{{ value }}</text>
							<text class="fullreduce-textline" :class="{'theLast': iindex == item.selfMadeGoodFullReduceArr.slice(0, 3).length - 1}"></text>
						</template>
					</view> -->
					<!-- <view class="goods-info-more">
				...
				<!-- 商品图片 -->
				<view class="flex">
					<view class="" style="margin-right: 6rpx; position: relative;" v-for="(v,index) in item.selfMadeGoods" :key="index">	
...
export default {
  ...
  methods: {
        gotoStore(item) {
			...
			if(this.indexID == 6) {
				uni.navigateTo({
					url: '/pages/merchantsDoself/storeHome/store-home?id=' + item.id
				});
			}else {
				uni.navigateTo({
