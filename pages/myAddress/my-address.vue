<template>
	<view class="my-address" :style="{minHeight: windowHeight + 'px'}">
		
		<view class="" style="height: 80rpx;display: flex; width: 750rpx; align-items: center; justify-content: flex-end; border-bottom: 0.1rpx solid #e3e3e3;"  @click="initLocation()">
			<image style="width: 36rpx;" src="../../static/mine/cur_location.png" mode="widthFix"></image>
			<text style="margin-left: 4rpx; font-size: 28rpx; margin-right: 34rpx; color: #1E90FF;">重新定位</text>
		</view>
		<view v-if="!isLoading" class="">
			<view v-if="addressList.length" class="adress-list">
				<view class="adress-li" @click="clickItem(item)" v-for="(item,i) of addressList" :key="i">
...
<script>
	...
	export default {
		...
		methods: {
					...
					success: (res) => {
						...
						if (res.errMsg == "getLocation:ok") {
							...
							plus.maps.Map.reverseGeocode(
								point, {},
								function(event) {
									prevPage.$vm.myLocation = event.address;
							...
							prevPage.$vm.listPortForm.longitude = res.longitude;
							prevPage.$vm.listPortForm.latitude = res.latitude;
							prevPage.$vm.shopList = [];
							prevPage.$vm.listPortForm.pageNum = 1;
			...
			clickItem(item) {
				if (this.from == 'home' || this.from == 'search') {
					// console.log(item)
					let pages = getCurrentPages(); //获取跳转的所有页面
					// let nowPage = pages[pages.length - 1]; //当前页
					let prevPage = pages[pages.length - 2]; //上一页
					// prevPage.$vm //上一页的this
					prevPage.$vm.listPortForm.longitude = item.longitude;
					prevPage.$vm.listPortForm.latitude = item.latitude;
					prevPage.$vm.myLocation = item.addressTitle;
					prevPage.$vm.shopList = [];
					prevPage.$vm.listPortForm.pageNum = 1;
					...
					// 存储经纬度
					let myLocation = {};
					myLocation.longitude = item.longitude;
					myLocation.latitude = item.latitude;
					myLocation.address = item.addressTitle
					uni.setStorageSync('location_key', myLocation);
			...
			editAdress(id) {
				...
				// })
				uni.navigateTo({
					url: '../orderForm/shippingAddress/edit-address?id=' + id
				})
			},
			// 获取地址列表
			getMyAddressList() {
			...
			addAdress() {
				...
				// })
				uni.navigateTo({
					url: '../orderForm/shippingAddress/shipping-address'
				})
			}
		},
		onPageScroll(e) {
