<template>
	<view class="invoice-helper">
		...
		<!-- 不需要发票 -->
		<view class="no-helper" @click="isNotNeed()">
			<!-- <image style="width: 22rpx;"
				:src="!isHelper?'../../../static/shopCar/not_select.png':'../../../static/shopCar/select.png'"
				mode="widthFix"></image> -->
			<text>不需要发票</text>
		</view>
		<!-- 标头：发票信息 -->
	...
	export default {
		...
		data() {
			return {
				...
				// helperList: [
				// 	{
				// 		companyName: '永信国际投资（集团）有限公司',
				// 		num: '税号1234567890',
				// 		isMoren: true
				// 	},
				// 	{
				// 		companyName: '某某国际投资（集团）有限公司',
				// 		num: '税号1234567890',
				// 		isMoren: false
				// 	}
				// ],
		...
		methods: {
			...
			isNotNeed() {
				...
				prevPage.$vm.orderList[this.shopIndex].userCarsVo.invoiceHeader = ''
				prevPage.$vm.orderList[this.shopIndex].userCarsVo.invoiceHeaderType = ''
				prevPage.$vm.orderList[this.shopIndex].userCarsVo.invoiceId = ''
				prevPage.$vm.orderList[this.shopIndex].userCarsVo.invoiceTax = ''
				uni.navigateBack({
					delta: 1
				})
			},
			...
			bindClick(item,e) {
				...
				switch (e.content.text) {
					...
					case '编辑':
					uni.navigateTo({
						url: './add-invoice-title?ids=' + item.id
					})
						break;
					default:
						break;
			...
			toUse(item) {
				...
				let prevPage = pages[pages.length - 2]; //上一页
				// prevPage.$vm //上一页的this
				prevPage.$vm.orderList[this.shopIndex].userCarsVo.invoiceHeader = item.invoiceHeader
				prevPage.$vm.orderList[this.shopIndex].userCarsVo.invoiceHeaderType = item.invoiceHeaderType
				prevPage.$vm.orderList[this.shopIndex].userCarsVo.invoiceId = item.id
				prevPage.$vm.orderList[this.shopIndex].userCarsVo.invoiceTax = item.taxNo
				uni.navigateBack({
					delta: 1
				})
			},
