	...
	<view class="order-form">
		...
		<view class="customer-info">
			<view class="flex-center-center">
				<text class="address">{{ orderDetails.takeOutOrder.address}}</text>
				<!-- <image style="width: 12rpx;" src="/static/shopCar/order_r.png" mode="widthFix"></image> -->
			</view>
			<view class="c-info">
				<text>{{ orderDetails.takeOutOrder.addressName }}</text>
				<text style="margin-left: 8rpx;">{{ orderDetails.takeOutOrder.addressSex == '0'? '先生': '女士' }}</text>
				<text style="margin-left: 28rpx;">{{ orderDetails.takeOutOrder.addressTel }}</text>
			</view>
		</view>
		<!-- 订单信息 -->
		<view class="order-info">
			<view class="order-item" v-for="(item,index) in 1" :key="index">
				...
				<view class="goods-info">
					...
					<view class="row5 flex-center-center" style="margin-bottom: 20rpx;">
						<view class="flex-center-center">
						...
						<view class="flex-center-center">
							<text class="cF75732">-</text><text class="cF75732" style="font-size: 22rpx;">￥</text><text
								class="cF75732"
								style="font-size: 32rpx;">{{ orderDetails.takeOutOrder.packageReduce? orderDetails.takeOutOrder.packageReduce :0 }}</text>
						</view>
					</view>
					<view class="row5 flex-center-center">
						...
						<view class="flex-center-center">
							<text class="cF75732">-</text><text class="cF75732" style="font-size: 22rpx;">￥</text><text
								class="cF75732"
								style="font-size: 32rpx;">{{ orderDetails.takeOutOrder.couponReduce? orderDetails.takeOutOrder.couponReduce:0 }}</text>
						</view>
					</view>
					<view class="row6 flex-center-center">
						...
						<view class="flex-center-center">
							<text
								class="text">{{ orderDetails.takeOutOrder.remarks?orderDetails.takeOutOrder.remarks:'无' }}</text>
							<!-- <image style="width: 12rpx;" src="/static/shopCar/order_r.png" mode="widthFix"></image> -->
						</view>
					</view>
					<view class="row6 flex-center-center">
						...
						<view class="flex-center-center">
							<text
								class="text">{{ orderDetails.takeOutOrder.tableware?orderDetails.takeOutOrder.tableware + '份':'无需餐具' }}</text>
							<!-- <image style="width: 12rpx;" src="/static/shopCar/order_r.png" mode="widthFix"></image> -->
						</view>
					</view>
				</view>
				...
				<view class="accounts">
					...
					<view class="">
						<text>已优惠：</text><text
							...
							style="font-weight: 500; font-size: 24rpx; color: #F75732;">{{ orderDetails.takeOutOrder.reduceMoney + orderDetails.takeOutOrder.couponReduce + orderDetails.takeOutOrder.packageReduce }}</text>
						<text style="margin-left: 20rpx;">小计</text><text
							style="font-weight: 500; font-size: 30rpx;">￥</text><text
							style="font-weight: 500; font-size: 44rpx;">{{ orderDetails.takeOutOrder.productAmountTotal }}</text>
					</view>
				</view>
			</view>
		</view>
		...
		<view class="submit">
			<view class="left">
				<text>已优惠￥{{ orderDetails.takeOutOrder.reduceMoney + orderDetails.takeOutOrder.couponReduce + orderDetails.takeOutOrder.packageReduce }}</text>
				<view class="flex-center-center">
					<text style="margin-right: 6rpx;">合计</text><text
						style="font-size: 32rpx; font-weight: 500;">￥{{ orderDetails.takeOutOrder.productAmountTotal }}</text><text
						style="font-size: 40rpx; font-weight: 500;"></text>
						
				</view>
			</view>
			<view v-if="orderDetails.takeOutOrder.status == 0" class="right" @click="togglePay()">
				<text>去支付</text>
			</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			getOrderDetails(id) {
				orderForm.getOrderDetails(id).then(res => {
					if (res.code == 200) {
