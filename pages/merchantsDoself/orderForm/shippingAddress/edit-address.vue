...
<script>
	...
	export default {
		...
		methods: {
			...
			// },
			// 获取地址详情
			getSelfMadeGoodUserAddress(id) {
				merchantsDoself.getSelfMadeGoodUserAddress(id).then(res => {
					if (res.code == 200) {
						this.userAddress.def = res.data.def
						this.userAddress.flag = res.data.flag
						this.userAddress.id = res.data.id
						this.userAddress.latitude = res.data.latitude
						this.userAddress.longitude = res.data.longitude
						this.userAddress.mobile = res.data.mobile
						this.userAddress.sex = res.data.sex
						this.userAddress.userName = res.data.userName
						this.userAddress.addressNum = res.data.addressNum
						this.userAddress.address = res.data.address
						this.userAddress.addressTitle = res.data.addressTitle
						this.userAddress.location = formatLocation(res.data.longitude, res.data.latitude)
			...
			saveAddress() {
				...
				// })
				this.editSelfMadeGoodUserAddress()
			},
			// 编辑地址
			editSelfMadeGoodUserAddress() {
				...
				merchantsDoself.editSelfMadeGoodUserAddress(this.userAddress).then(res => {
					if (res.code == 200) {
						uni.showToast({
							title: "编辑成功",
							icon: 'success'
			...
			// 删除
			deleteAddress(id){
				merchantsDoself.deleteAddress(id).then(res => {
					if (res.code == 200) {
						uni.showToast({
							title: "删除成功",
							icon: 'success'
		...
		onLoad(options) {
			...
			this.getSelfMadeGoodUserAddress(options.id);
			// this.$nextTick(res => {
			// 	this.init()
			// })
		},
