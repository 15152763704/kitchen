...
<script>
	...
	export default {
		...
		methods: {
			...
			getShopInfo() {
				shopApply.getShopById().then(res => {
					if (res.code == 200) {
						...
						this.typeIdSide = res.data.typeIdSide;
						this.getTreeselect();
					}
				}).catch(err => {
					uni.showToast({
			...
			toNext() {
				uni.navigateTo({
					url: './edit-open?typeId=' + this.selectedList[0].shopTypeId + '&typeIdSide=' + this.selectedList[1].shopTypeId + '&typeName=' + this.selectedList[0].shopTypeName + '&typeNameSide=' + this.selectedList[1].shopTypeName
				})
				// uni.showModal({
					...
					if (res.code == 200) {
						...
						for (var i = 0; i < res.data.length; i++) {
							...
							for (var j = 0; j < res.data[i].children.length; j++) {
								if(this.typeId == res.data[i].children[j].shopTypeId) {
									res.data[i].children[j].isChecked = true;
									this.parentId = res.data[i].children[j].parentId;
									this.selectedList.push(res.data[i].children[j])
								}
								if(this.typeIdSide == res.data[i].children[j].shopTypeId) {
									res.data[i].children[j].isChecked = true;
									this.selectedList.push(res.data[i].children[j])
								}
							}
							// parentId
							if(this.parentId == res.data[i].shopTypeId) {
								res.data[i].isChecked = true;
								this.level2List = res.data[i].children;
							}
						}
