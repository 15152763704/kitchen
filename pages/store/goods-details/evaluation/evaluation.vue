...
<script>
	...
	export default {
			...
			onLoad(option) {
				...
				this.getShopEvalList(option.shopId, '', '', option.foodId);
				this.findShopEvalStatics(option.shopId, '', option.foodId);
			},
			mounted() {
				uni.getSystemInfo({
					success: (res) => {
			...
			methods: {
				getEvalFilterType(type) {
					...
					this.getShopEvalList(this.shopId, recType, '', this.foodId);
				},
				findShopEvalStatics(shopId, evalType, foodId) {
